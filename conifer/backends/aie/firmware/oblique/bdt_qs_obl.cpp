...
// The templates live here, not in the header: aiecompiler's frontend introspects a
// kernel's argument types from its declaration, and a template instantiation defeats it.
//
// always_inline on the closure, not only on the body it is passed to: left standing as a
// real function on one tile of a multi-tile build, the row reader costs that tile call
// overhead its neighbours do not pay, and the slowest tile sets the reported number.
...
// At N_TILES == 1 this is the single-tile kernel's arithmetic exactly; under sample-split
// every tile runs it, on its own samples.
