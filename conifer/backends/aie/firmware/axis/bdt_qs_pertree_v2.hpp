...
// Word q of a bitvector, at compile time. The argument widens to uint64 whatever
// bv_t is, so the shift is defined at every depth.
...
constexpr bvw_t index_bit_mask(unsigned p) {
    ...
    for (unsigned q = 0; q < BV_BITS; q++)
        if ((q >> p) & 1u) mask |= (bvw_t)((bvw_t)1 << q);
    return mask;
}

inline constexpr std::array<bvw_t, IN_WORD_BITS> IDX_MASK = [] {
    std::array<bvw_t, IN_WORD_BITS> a{};
    for (unsigned p = 0; p < IN_WORD_BITS; p++) a[p] = index_bit_mask(p);
    return a;
}();

using vrow = aie::vector<leaf_t, LEAF_SLOTS>;

alignas(sizeof(leaf_t) * LEAF_SLOTS)
inline constexpr std::array<leaf_t, bdtm::N_TREES * LEAF_SLOTS> LEAVES_P = [] {
...
inline vleaf mux(const vrow &row, const vmask *mbit) {
    static_assert(N >= 2 && (N & (N - 1)) == 0);
    constexpr unsigned BIT = log2_of(N) - 1;
    if constexpr (N == 2) {
        // The bottom level takes its two candidates as elements of a vector already in
        // a register, so the pair collapses to one vector with no scalar load at all.
...
__attribute__((always_inline))
inline vleaf leaf_value(const vbvw v[BV_WORDS], unsigned h) {
    const vbvw zero = aie::zeros<bvw_t, W>();

    // The isolated lowest set bit of the lowest non-zero word, and that word's index.
    // Folded from the top down so a lower non-zero word overrides the words above it.
...
#pragma unroll
    ...
    // Which word won, as index bits above the in-word ones. With BV_WORDS == 2 this
    // is a single bit: set iff word 0 was empty.
#pragma unroll
...
constexpr unsigned qt_lanes() {
    unsigned n = 1;
    while (n < bdtm::QS_NODES_PER_TREE) n <<= 1;
    return n < MIN_QT_LANES ? MIN_QT_LANES : n;
}
constexpr unsigned QT_LANES = qt_lanes();  // depth 4: 16, one 256-bit vector of int16
...
inline constexpr std::array<int16_t, QT_N + QT_LANES> QT_THR_P = [] {
    ...
    for (unsigned i = 0; i < QT_N; i++) a[i] = bdtm::QT_THR[i];
    return a;
}();
...
inline vscore qs_score_group(LoadRow load_row) {
    ...
    vfeat x[N_FEAT];
    for (unsigned k = 0; k < N_FEAT; k++) x[k] = load_row(k);
